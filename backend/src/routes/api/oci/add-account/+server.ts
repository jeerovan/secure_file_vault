...
export const POST: RequestHandler = async ({ request }) => {
	...
	const { app_id, app_key, bucket, namespace, region } = body;

	if (!app_id || !app_key || !bucket || !namespace || !region) {
		return json({ success: 0, message: ErrorCode.MISSING_FIELDS });
	}
	const validData = await verifyR2Credentials(bucket, namespace, app_id, app_key, region);
	if (validData) {
		const credentials = {
			appId: app_id,
			appKey: app_key,
		...
		await addCredentials(authUser.id, app_id, credentials, provider);
